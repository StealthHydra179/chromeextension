...
(function () {
  ...
  const LobiboxBase = {
    ...
    _calculatePosition: function (position) {
      ...
      const left = ($(window).outerWidth() - me.$el.outerWidth()) / 2
      return {
        left,
        top
      }
    ...
    _createButton: function (type, op) {
      const me = this
      const btn = $('<button></button>').addClass(op.class).attr('data-type', type).html(op.text)
    ...
    _generateButtons: function () {
      const me = this
      const btns = []
      for (const i in me.$options.buttons) {
        if (me.$options.buttons.hasOwnProperty(i)) {
          const op = me.$options.buttons[i]
          const btn = me._createButton(i, op)
          btns.push(btn)
        }
      }
      return btns
    ...
    _createMarkup: function () {
      const me = this
      const lobibox = $('<div class="lobibox"></div>')
      lobibox.attr('data-is-modal', me.$options.modal)
      const header = $('<div class="lobibox-header"></div>').append('<span class="lobibox-title"></span>')
      const body = $('<div class="lobibox-body"></div>')
      lobibox.append(header)
      lobibox.append(body)
      if (me.$options.buttons && !$.isEmptyObject(me.$options.buttons)) {
        const footer = $('<div class="lobibox-footer"></div>')
    ...
    _calculateBodyHeight: function (height) {
      const me = this
      const headerHeight = me.$el.find('.lobibox-header').outerHeight()
      const footerHeight = me.$el.find('.lobibox-footer').outerHeight()
      return height - (headerHeight || 0) - (footerHeight || 0)
    },
    ...
    _addCloseButton: function () {
      const me = this
      const closeBtn = $('<span class="btn-close">&times;</span>')
    ...
    _position: function () {
      ...
      me._setSize()
      const pos = me._calculatePosition()
    ...
    _enableDrag: function () {
      const el = this.$el
      const heading = el.find('.lobibox-header')
      ...
      $(document).on('mousemove.lobibox', function (ev) {
        if (el.attr('allow-drag') === 'true') {
          const left = ev.clientX - parseInt(el.attr('offset-left'), 10) - parseInt(el.css('border-left-width'), 10)
          const top = ev.clientY - parseInt(el.attr('offset-top'), 10) - parseInt(el.css('border-top-width'), 10)
          el.css({
            left,
            top
          })
    ...
    show: function () {
      ...
      if (me.$options.buttons) {
        const buttons = me.$el.find('.lobibox-footer').children()
        buttons[0].focus()
      }
      if (me.$options.modal) {
  ...
  LobiboxPrompt.prototype = $.extend({}, LobiboxBase, {
    ...
    _processInput: function (options) {
      ...
      const mergedOptions = LobiboxBase._processInput.call(me, options)
    ...
    isValid: function () {
      const me = this
      const $error = me.$el.find('.lobibox-input-error-message')
  ...
  LobiboxProgress.prototype = $.extend({}, LobiboxBase, {
    ...
    _afterShow: function () {
      ...
      const innerHTML = $('<div></div>').append(label, me.$progressBarElement)
    ...
    _createProgressbar: function () {
      const me = this
      const outer = $('<div class="lobibox-progress-bar-wrapper lobibox-progress-outer"></div>').append(
...
var Lobibox = Lobibox || {};
(function () {
  const LobiboxNotify = function (type, options) {
    // ------------------------------------------------------------------------------
    // ----------------PROTOTYPE VARIABLES-------------------------------------------
    // ------------------------------------------------------------------------------
    this.$type = null
    ...
    // -----------------PRIVATE FUNCTIONS--------------------------------------------
    // ------------------------------------------------------------------------------
    const _processInput = function (options) {
    ...
    const _appendInWrapper = function ($el, $wrapper) {
      if (me.$options.size === 'normal') {
        ...
        const tabPane = _createTabPane().append($el)
        const $li = _createTabControl(tabPane.attr('id'))
    ...
    var _activateTab = function ($li) {
      ...
      $li.addClass('active')
      const $current = $($li.find('>a').attr('href'))
    ...
    const _createNotifyWrapper = function () {
      const selector =
    ...
    const _createNotify = function () {
      const OPTS = Lobibox.notify.OPTIONS
      let $iconEl
      let $innerIconEl
      let $iconWrapper
      let $body
      let $msg
      const $notify = $('<div></div>', {
    ...
    var _addDelay = function ($el) {
      ...
      var timer = setInterval(function () {
        ...
        let width = (100 * time) / me.$options.delay
        if (width >= 100) {
          width = 100
          me.remove()
    ...
    const _findTabToActivate = function ($li) {
      let $itemToActivate = $li.prev()
    ...
    this.remove = function () {
      me.$el.removeClass(me.$options.showClass).addClass(me.$options.hideClass)
      const parent = me.$el.parent()
      const wrapper = parent.closest('.lobibox-notify-wrapper-large')

      const href = '#' + parent.attr('id')

      const $li = wrapper.find('>.lb-notify-tabs>li:has(a[href="' + href + '"])')
      $li.addClass(Lobibox.notify.OPTIONS.class).addClass(me.$options.hideClass)
      setTimeout(function () {
        if (me.$options.size === 'normal' || me.$options.size === 'mini') {
          me.$el.remove()
        } else if (me.$options.size === 'large') {
          const $newLi = _findTabToActivate($li)
          if ($newLi) {
            _activateTab($newLi)
          }
          $li.remove()
        ...
        const list = Lobibox.notify.list
        const ind = list.indexOf(me)
        list.splice(ind, 1)
        const next = list[ind]
    ...
    me._init = function () {
      ...
      me.$el = $notify
      if (me.$options.sound) {
        const snd = new Audio(me.$options.sound) // buffers automatically when created
        snd.play()
      }
  ...
  Lobibox.notify = function (type, options) {
    if (['default', 'info', 'warning', 'error', 'success'].indexOf(type) > -1) {
      const lobibox = new LobiboxNotify(type, options)
