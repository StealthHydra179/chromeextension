...
(function ($) {
  ...
  const GetDisplayFilesize = function (numbytes, adjustprecision, units) {
    ...
    const pos = Math.floor(Math.log(numbytes) / Math.log(magnitude))
    const result = (numbytes / Math.pow(magnitude, pos))

    return (pos == 0 || (adjustprecision && result >= 99.995) ? result.toFixed(0) : result.toFixed(2)) + ' ' + abbreviations[pos]
  }

  const DisplayPreviewDialog = function (preview, endelem, inforow, data, settings) {
    const previewbackground = $('<div>').addClass('ff_fileupload_dialog_background')
    const previewclone = preview.clone(true, true).click(function (e) {
      e.stopPropagation()
    })
    const previewdialog = $('<div>').addClass('ff_fileupload_dialog_main').append(previewclone)

    const HidePreviewDialog = function () {
  ...
  const InitShowAriaLabelInfo = function (inforow) {
    inforow.find('button').hover(function () {
      const val = $(this).attr('aria-label')
  ...
  $.fn.FancyFileUpload = function (options) {
    this.each(function () {
      ...
      if ($this.data('fancy-fileupload') && typeof ($this.data('fancy-fileupload')) === 'object') {
        ...
        const data = $this.data('fancy-fileupload')
    ...
    const settings = $.extend({}, $.fn.FancyFileUpload.defaults, options)
    ...
    $(window).on('beforeunload.fancy_fileupload', function (e) {
      const active = $('.ff_fileupload_uploading, .ff_fileupload_starting')
      const queued = $('.ff_fileupload_queued')
    ...
    const AddFile = function (uploads, e, data) {
      ...
      const StartUpload = function (e) {
        e.preventDefault()

        // Set filename.
        if (settings.edit && !data.ff_info.errors.length) {
          const fileinput = inforow.find('.ff_fileupload_filename input')
          if (fileinput.length) {
    ...
    const UploadProgress = function (e, data) {
      const progress = (data.total < 1 ? 0 : data.loaded / data.total * 100)
    ...
    const UploadFailed = function (e, data) {
    ...
    const UploadDone = function (e, data) {
    ...
    const UploadChunkSend = function (e, data) {
      if (data.ff_info) {
    ...
    const UploadChunkDone = function (e, data) {
    ...
    return this.each(function () {
      ...
      const form = $('<form>').addClass('ff_fileupload_hidden').attr({
        action: settings.url,
        method: 'post',
      ...
      if ($this.attr('accept')) {
        ...
        if (!settings.accept) {
          const accept = $this.attr('accept').split(',')

          settings.accept = []
          for (var x = 0; x < accept.length; x++) {
            const opt = $.trim(accept[x]).toLowerCase()
      ...
      const fileuploadwrap = $('<div>').addClass('ff_fileupload_wrap')
      ...
      const dropzonewrap = $('<div>').addClass('ff_fileupload_dropzone_wrap')
      const dropzone = $('<button>').addClass('ff_fileupload_dropzone').attr('type', 'button').attr('aria-label', Translate('Browse, drag-and-drop, or paste files to upload'))
      ...
      const dropzonetools = $('<div>').addClass('ff_fileupload_dropzone_tools')
      dropzonewrap.append(dropzonetools)

      // Record audio.
      if (settings.recordaudio && navigator.mediaDevices && window.MediaRecorder) {
        const audiobutton = $('<button>').addClass('ff_fileupload_dropzone_tool').addClass('ff_fileupload_recordaudio').attr('type', 'button').attr('aria-label', Translate('Record audio using a microphone'))
      ...
      if (settings.recordvideo && navigator.mediaDevices && window.MediaRecorder) {
        const videobutton = $('<button>').addClass('ff_fileupload_dropzone_tool').addClass('ff_fileupload_recordvideo').attr('type', 'button').attr('aria-label', Translate('Record video using a camera'))
        dropzonetools.append(videobutton)

        const videorecpreview = $('<video>').prop('muted', true).prop('autoplay', true).addClass('ff_fileupload_recordvideo_preview').addClass('ff_fileupload_hidden')
      ...
      const uploads = $('<table>').addClass('ff_fileupload_uploads')
