      ...
      a(
        ...
        { colorByPoint: !0, dataLabels: { y: 0 } },
        {
          ...
          drawPoints: function () {
            ...
            const g = c.center
            const b = a.options
            const m = a.chart.renderer
            const d = b.overshoot
            const h = v(d) ? (d / 180) * Math.PI : 0
            let k
            v(b.threshold) &&
              (k =
                c.startAngleRad +
            ...
            a.points.forEach(function (f) {
              if (!f.isNull) {
                let d = f.graphic
                let e = c.startAngleRad + c.translate(f.y, null, null, null, !0)
                let k = (w(t(f.options.radius, b.radius, 100)) * g[2]) / 200
                let n =
                    (w(t(f.options.innerRadius, b.innerRadius, 60)) * g[2]) /
                    200
                let p = c.toColor(f.y, f)
                let q = Math.min(c.startAngleRad, c.endAngleRad)
                const l = Math.max(c.startAngleRad, c.endAngleRad)
                p === 'none' && (p = f.color || a.color || 'none')
                p !== 'none' && (f.color = p)
                e = r(e, q - h, l + h)
                !1 === b.wrap && (e = r(e, q, l))
                ...
                d
                  ...
                  : (f.graphic = d =
                      m.arc(n).attr({ fill: p, 'sweep-flag': 0 }).add(a.group))
                a.chart.styledMode ||
                  (b.linecap !== 'square' &&
                    d.attr({
