...
(function (factory) {
  ...
  TimePicker.prototype.now = function (type, value /*, options */) {
    ...
    const date = new Date()
    let nowMinutes = date.getHours() * MINUTES_IN_HOUR + date.getMinutes()
    const isValueInteger = _.isInteger(value)
    let isBelowInterval
  ...
  TimePicker.prototype.validate = function (type, timeObject, options) {
    const clock = this
    const interval = options && options.interval ? options.interval : clock.item.interval
  ...
  TimePicker.prototype.disabled = function (timeToVerify) {
    ...
    let isDisabledMatch = clock.item.disable.filter(function (timeToDisable) {
  ...
  TimePicker.prototype.shift = function (timeObject, interval) {
    const clock = this
    const minLimit = clock.item.min.pick
    const maxLimit = clock.item.max.pick /*,
        safety = 1000 */
  ...
  TimePicker.prototype.deactivate = function (type, timesToDisable) {
    const clock = this
    let disabledItems = clock.item.disable.slice(0)
  ...
  TimePicker.prototype.activate = function (type, timesToEnable) {
    const clock = this
    let disabledItems = clock.item.disable
    ...
    // Otherwise go through the disabled times.
      ...
      timesToEnable.map(function (unitToEnable) {
        let matchFound, disabledUnit, index, isRangeMatched
  ...
  TimePicker.prototype.nodes = function (isOpen) {
    ...
    const disabledCollection = clock.item.disable
    ...
    return _.node(
      ...
      _.group({
        ...
        item: function (loopedTime) {
          ...
          const isDisabled = disabledCollection && clock.disabled(loopedTime)
