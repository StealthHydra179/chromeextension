...
(function ($) {
  ...
  const DisplayPreviewDialog = function (
    preview,
    endelem,
    inforow,
    data,
    ...
    const previewbackground = $('<div>').addClass(
  ...
  $.fn.FancyFileUpload = function (options) {
    ...
    const AddFile = function (uploads, e, data) {
      ...
      const fileextclass = alphanum.charAt(
      ...
      const StartUpload = function (e) {
        ...
        if (settings.startupload) { settings.startupload.call(inforow, SubmitUpload, e, data) } else SubmitUpload()
      }
    ...
    const UploadFailed = function (e, data) {
      ...
      if (data.ff_info.removewidget) {
        ...
        // Set the error info.
        if (data.errorThrown === 'abort') { data.ff_info.errors.push(Translate('The upload was cancelled.')) } else if (data.errorThrown === 'failed_with_msg') {
          data.ff_info.errors.push(
            FormatStr(
